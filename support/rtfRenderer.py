...
""".encode('utf-8'))
    ...
    def render_heading_template(self, token, size):
        self.end_par()
        self.write(u'\n{\\pard\\sb360\\sa360\\fs' + str(size) + ' ' + token.getValue() + u'\n')
    ...
    def render_f_s(self, token):     self.write(ur'{\super\chftn}{\footnote\pard\plain\chftn ')
    def render_f_e(self, token):     self.write(ur' }')
    def render_x_s(self, token):     self.write(ur'{\super\chftn}{\footnote\pard\plain\chftn ')
    def render_x_e(self, token):     self.write(ur' }')
    
    def render_nd_s(self, token):     self.write(ur'{\scaps ')
    def render_nd_e(self, token):     self.write(ur'}')
    
    def render_b(self, token):        self.end_par() ; self.write(u'\n{\\pard \n')
    
    def render_em_s(self, token):     self.write(u'{\i ')
    def render_em_e(self, token):     self.write(u'}')
        
    def render_d(self, token):        self.render_m(token) ; self.write('{\i ' + token.getValue() + '}')

    # Que selah selah
    def render_qs_s(self, token):     self.write(u'{\i ')
    def render_qs_e(self, token):     self.write(u'}')
